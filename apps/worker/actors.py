"""
Dramatiq actors для выполнения Job.

Определяет actor execute_job, который:
1. Десериализует Job из dict
2. Создаёт WorkerRegistry и регистрирует воркеры
3. Создаёт WorkerRunner и оборачивает его в RetryableRunner
4. Выполняет job и возвращает JobResult в виде dict
"""
...
@dramatiq.actor
def execute_job(job_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
    Dramatiq actor для выполнения Job.
